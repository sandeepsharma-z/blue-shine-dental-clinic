...
const botReply = (input: string) => {
  ...
  if (/(hi|hello|salam|assalam|hey)/.test(q)) {
    return "Hello! Main Blue Shine Dental Clinic assistant hoon. Aap implant, pain, braces, cleaning, timing ya booking ke baare me pooch sakte hain.";
  }
  ...
  if (/(pain|dard|toothache|sujaan|swelling)/.test(q)) {
    return "Agar severe pain ya swelling ho to delay mat karein. Garam-thanda avoid karein aur emergency checkup ke liye clinic visit karein ya call karein: 095995 82158.";
  }
  ...
  if (/(appointment|book|booking|schedule)/.test(q)) {
    return "Appointment ke liye page par Book Now form fill karein ya direct call karein: 095995 82158.";
  }
  ...
  if (/(emergency|urgent|bleeding|broken tooth)/.test(q)) {
    return "Emergency case me foran contact karein: 095995 82158. Hum urgent dental pain, fracture aur infection cases handle karte hain.";
  }
